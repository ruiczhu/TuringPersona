...
def apply_personality_traits(conversations, transformer, trait_sample, trait_adjustments=None, num_profiles=1,
...
def save_transformed_conversations(results, output_path):
    ...
    framework = PersonalityFramework()
    # 创建表头行 - 只保留基础信息和五个维度
    header = ['conversation_id', 'profile_id', 'role', 'content', 'original_content', 'turn', 'position',
              'O', 'C', 'E', 'A', 'N']
            ...
            for profile_id, profile_data in data['transformed'].items():
                ...
                for i, turn in enumerate(transformed_turns):
                    ...
                    row = [
                        conv_id,  # 对话ID
                        profile_id,  # 人格配置ID
                        turn['role'],  # 角色（user或assistant）
                        original_turns[i]['content'],  # 原始对话内容
                        turn['content'],  # 转换后的对话内容
                        original_turns[i]['turn'],  # 原始对话的轮次编号
                        original_turns[i]['position']  # 原始对话的位置
                    ]
